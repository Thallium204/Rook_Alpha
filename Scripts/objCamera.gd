...
func clampCameraPosition():
	if Globals.moveStructureMode != "moving":
		...
		position.x = clamp( position.x , (vp_dim[0] / 2) * zoom.x , bg_dim[0] - ((vp_dim[0] / 2) * zoom.x) )
		position.y = clamp( position.y , (vp_dim[1] / 2) * zoom.y , bg_dim[1] - ((vp_dim[1] / 2) * zoom.y) )
	else:
		position.x = clamp( position.x , 0 , bg_dim[0] )
		position.y = clamp( position.y , vertical_offset , bg_dim[1] )
...
func _input(event):
	...
	if event is InputEventScreenTouch:
		...
		elif events.size() == 2 and zoomEnabled == true:
